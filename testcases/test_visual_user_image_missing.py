...
def test_visual_user_image_missing(driver, test_params, request):
    ...
    for item in params['items']:
        ...
        #                                                        "Item page"
        if img_src_dict[item][0] == img_src_dict[item][1]:
            logger.warn(f"Actual and Expected Images does not match for {item}")
